--[[
    PET VIEWER - Versão Final
    Dropdown profissional • UI menor • Status com ícones e cores
]]
...
local PERK_CONFIG = {
    ["strength"] = {
        icon = "💪",
        name = "Strength",
        textColor = Color3.fromRGB(255, 230, 0),   -- Amarelo
        borderColor = Color3.fromRGB(0, 0, 0)
    },
    ["durability"] = {
        icon = "🛡️",
        name = "Durabilidade",
        textColor = Color3.fromRGB(0, 150, 255),   -- Azul
        borderColor = Color3.fromRGB(0, 0, 0)
    },
    ["agility"] = {
        icon = "⚡",
        name = "Agility",
        textColor = Color3.fromRGB(0, 255, 80),    -- Verde
        borderColor = Color3.fromRGB(0, 0, 0)
    },
    ["damage"] = {
        icon = "⚔️",
        name = "Damage",
        textColor = Color3.fromRGB(255, 50, 50),   -- Vermelho
        borderColor = Color3.fromRGB(0, 0, 0)
    }
}
...
-- Sistema de sufixo (resultado real)
...
local function getPetData(petInst)
    ...
    local strVal = petInst:FindFirstChild("perksFolder") and petInst.perksFolder:FindFirstChild("strength")
    ...
    return {
        ...
        strength  = strVal and tonumber(strVal.Value) or 0,
...
-- Botão X (Fechar) - quadrado vermelho, X branco, borda preta
...
-- Botão Minimizar - mesmo estilo (quadrado vermelho, X branco, borda preta)
local MinBtn = Instance.new("TextButton")
MinBtn.Size = UDim2.new(0, 22, 0, 22)
MinBtn.Position = UDim2.new(1, -54, 0, 5)
MinBtn.BackgroundColor3 = Color3.fromRGB(210, 40, 45)
MinBtn.Text = "−"
...
-- Dropdown profissional (sem seta, sem borda azul)
...
-- Lista do Dropdown (profissional, sem bordas azuis)
...
-- Botão X do Status (mesmo estilo: vermelho, X branco, borda preta)
...
-- Grid dos pets (mesmo formato de antes)
...
local function loadPlayerPets()
    ...
    for order, data in ipairs(petList) do
        ...
        createStroke(PetCard, data.color, 1.8)

        -- Só imagem (sem animação)
        ...
        PetImg.Image = extractPetImage(data.inst)
        PetImg.ZIndex = 9
        ...
        PetCard.MouseButton1Click:Connect(function()
            ...
            -- Título apenas "Status" na cor da raridade
            StatusTitle.Text = "Status"
            StatusTitle.TextColor3 = data.color
            createStroke(StatusTitle, Color3.fromRGB(0, 0, 0), 1.5, Enum.ApplyStrokeMode.Contextual)
                ...
                for _, perk in pairs(perks:GetChildren()) do
                    if perk:IsA("ValueBase") then
                        ...
                        if conf then
                            ...
                            -- Nome + ícone (cor do perk + borda preta)
    ...
    PetScroll.CanvasSize = UDim2.new(0, 0, 0, (math.ceil(#petList / 3) * 117) + 10)
end
